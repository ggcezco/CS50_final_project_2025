...
def main():
    ...
    for result in results:
        ...
        price_display = ", ".join(
            [f"{site}: R$ {price:.2f}" if price is not None else f"{site}: N/A" for site, price in result['Prices'].items()]
        )
    ...
    if len(sys.argv) == 2 and sys.argv[1].lower().endswith(".csv"):
        ...
        with open(filename, mode='r', newline='', encoding='utf-8') as f:
            ...
            for i, row in enumerate(reader):
                if i < 10 and row and row[0].strip().isdigit() and (len(row[0]) == 8 or len(row[0]) == 13):
